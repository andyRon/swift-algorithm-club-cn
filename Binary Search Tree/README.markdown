...
> **注：** sort，平常都可以理解为从大到小或从小到大的排序，但本文有点不同，可以理解为是按一定规则的整理。因为“排序”在汉语中也不是简单理解为从大到小或从小到大，所以我还是把sort翻译成*排序*，但注意和平常理解的排序有点不同。
...
如果一个分支明显长于另一个分支，则搜索变得非常慢。 我们最终检查的花费超出了我们的需要。 在最坏的情况下，树的高度可以变为 *n*。 这样的树就像 [linked list](../Linked%20List/) 而不是二叉搜索树，性能降低到 **O(n)**。 不好！
